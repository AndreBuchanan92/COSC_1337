...
{		
	...
	{	
		...
		else if (option == 5)
		{
			...
			if (cds[0].getArtist().empty()==0)
			{
				// calling edit cd function to edit cd information
				editCD(cds);
			}
			// if file did not open then display following error message
			else
...
void editCD(CD cds[])
{
	//get index of the specific cd collection which want to modify
	int foundAt=findCD(cds);
	
	// if cd exist in the collection
	if(foundAt!=-1){
		cout << "Title: " << cds[foundAt].getTitle() << " and Artist: " << cds[foundAt].getArtist()  << " is found in the CD collection." << endl<<endl;	
		
		//declare variable
		string newArtistName, newTitleName;
		int newYearReleased;
		char editArtist,editTitle,editYearReleased;
		
		//check if user want to modify artist name
		cout<<"Do you want to edit Artist Name? (Y/N) : ";
		cin>>editArtist;
		if(editArtist=='Y' or editArtist=='y'){
			cout<<"Please enter new Artist name :";
			cin.ignore();
			getline(cin,newArtistName);
			cds[foundAt].setArtist(newArtistName);	
		}
		
		//check if user want to modify title
		cout<<"Do you want to edit Title Name? (Y/N) : ";
		cin>>editTitle;
		if(editTitle=='Y' or editTitle=='y'){
			cout<<"Please enter new Title name :";
			cin.ignore();
			getline(cin,newTitleName);
			cds[foundAt].setTitle(newTitleName);	
		}
	
		//check if user want to modify released year
		cout<<"Do you want to edit Year Released? (Y/N) :";
		cin>>editYearReleased;
		if(editYearReleased=='Y' or editYearReleased=='y'){
			cout<<"Please enter new Year Released :";
			cin>>newYearReleased;
			cds[foundAt].setYearReleased(newYearReleased);	
		}
		
		// display friendly message for collection updated
		cout << "Your CD collection has been updated."	<< endl << endl;	
	}
}
